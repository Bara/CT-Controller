void ConnectToSQL()
{
	SQL_TConnect(sqlConnect, "ct_controller");
}

void CreateTable()
{
	char sQuery[1024];
	Format(sQuery, sizeof(sQuery), "CREATE TABLE IF NOT EXISTS `ct_controller` (`id` INT NOT NULL AUTO_INCREMENT, `name` varchar(256) COLLATE utf8mb4_unicode_ci NOT NULL, `communityid` varchar(24) COLLATE utf8mb4_unicode_ci NOT NULL, `verify` tinyint(1) NOT NULL, `verifyAdmin` varchar(24) COLLATE utf8mb4_unicode_ci NOT NULL, `banned` tinyint(1) NOT NULL, `rounds` int(11) NOT NULL, `banAdmin` varchar(24) COLLATE utf8mb4_unicode_ci NOT NULL, `banReason` text COLLATE utf8mb4_unicode_ci NOT NULL, `unbanAdmin` varchar(24) COLLATE utf8mb4_unicode_ci NOT NULL, PRIMARY KEY (`id`), UNIQUE KEY (`communityid`)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;");
...
bool IsValidCT(int client, bool blockMessage = false)
{
	...
	if(!g_bVerify[client] && STAMM_GetClientPoints(client) < g_cStammPoints.IntValue)
	{
		if (!blockMessage)
		{
			LogMessage("\"%L\" tried to join ct! (Rejected: g_bVerify is false / STAMM_GetClientPoints (%d))", client, STAMM_GetClientPoints(client));
			CPrintToChat(client, "Sie sind  {lightblue}nicht freigeschaltet {lightgreen}(Melde dich dafür bitte  {lightblue}bei Sugi{lightgreen}.) und haben nicht genug Stammpunkte ({lightblue}ab %d Stammpunkte{lightgreen}).", g_cStammPoints.IntValue);
		}
		return false;
	}
...
void PerformDiscord(int client, int target, int rounds, const char[] reason)
{
	...
	EscapeString(sAdminName, strlen(sAdminName));
	
	char sMSG[2048] = MSG_BAN, sMap[32];
	...
	ReplaceString(sMSG, sizeof(sMSG), "{MAP}", sMap);
